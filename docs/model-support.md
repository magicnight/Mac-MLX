...
| Seed-OSS-36B | `seed_oss` | ✅ Tested | Dense GQA with independent attention/o-proj/MLP bias switches; full 1e-4 parity suite on real Metal, plus a real-checkpoint generation smoke at 18.2 tok/s on the 4-bit checkpoint. No chat-template override needed: the checkpoint's `chat_template.jinja` builds its thinking-budget table as an integer-keyed Jinja dict that swift-jinja 2.3.6 could not parse; swift-jinja ≥2.4.0 renders it natively (the parser limitation was reported and fixed — huggingface/swift-jinja #62 — and the former built-in rewrite was removed), proven byte-for-byte, ungated, by `SeedOssChatTemplateParityTests`. A per-model `macmlx.chat_template.jinja` file still overrides it |
| Hunyuan V1 Dense (0.5B–7B) | `hunyuan_v1_dense` | ✅ Tested | Dense GQA whose twists are per-head q/k RMSNorm applied AFTER RoPE (the inverse of the Qwen3 lineage's pre-RoPE norm) and a DynamicNTKAlpha RoPE whose base is pre-scaled by `rope_scaling.alpha`; a single `attention_bias` drives q/k/v/o and the embeddings are tied. Full 1e-4 parity suite on real Metal (two adversarial configs inverting every switch), plus a real-checkpoint generation smoke at 80.3 tok/s on the 1.8B 4-bit checkpoint. No chat-template override needed: the checkpoint's own `chat_template.jinja` renders natively under swift-jinja, proven byte-for-byte by `HunyuanV1DenseChatTemplateParityTests` — including the `<answer>`-tagged historical-assistant branch (`content.split('<answer>')[-1].strip('</answer>').strip()`), covered since the swift-jinja `strip(arg)` fix we reported (huggingface/swift-jinja #64) |
| Cohere Command R7B | `cohere2` | ✅ Tested | Cohere-family decoder with a PARALLEL residual block (attention and MLP both read the same single `input_layernorm(x)`; output is `attn + mlp + x`, no post-attention norm), `LayerNorm` (not RMSNorm) with a `layer_norm_bias` switch, tied embeddings scaled by `logit_scale`, and — the Command R7B twist — interleaved sliding-window / global attention on a `sliding_window_pattern`: traditional (GPT-J) RoPE runs only on the sliding-window layers while the global layers get NO positional encoding (NoPE), and the KV cache is mixed (RotatingKVCache on sliding layers, KVCacheSimple on global). Full 1e-4 parity suite on real Metal (two adversarial configs inverting every switch — bias, ln_bias, logit_scale, pattern, window — with seq_len > window so the sliding mask genuinely differs from full-causal), plus a real-checkpoint generation smoke at 21.7 tok/s on the 7B 4-bit checkpoint (`c4ai-command-r7b-12-2024` — the `12-2024` is the December 2024 release date, not a parameter count). No chat-template override needed: the checkpoint's `default` named template embeds a tool/RAG branch swift-jinja 2.3.6 could not parse; swift-jinja ≥2.4.0 parses and renders the whole template natively on the standard conversation path (the parser limitation was reported and fixed — huggingface/swift-jinja #63 — and the former built-in rewrite was removed), proven byte-for-byte, ungated, by `Cohere2ChatTemplateParityTests`. A per-model `macmlx.chat_template.jinja` file still overrides it |

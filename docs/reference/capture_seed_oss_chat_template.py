...
`capture_seed_oss.py` + `SeedOss*ParityTests`. This follow-up verifies that the
checkpoint's own `chat_template.jinja` renders correctly under swift-jinja (the
...
The Swift test `SeedOssChatTemplateParityTests` renders the SAME template through
swift-jinja and asserts byte-for-byte string equality — proving native rendering,
with NO model weights and NO Metal required.
...
def main() -> None:
    ...
    payload = {
        "_comment": (
            "Render-parity reference for the Seed-OSS checkpoint's OWN "
            "chat_template.jinja (no macMLX override needed as of swift-jinja "
            "2.4.0, which fixes the integer-keyed object-literal parse — "
            "huggingface/swift-jinja #62, reported by macMLX). Generated by "
            "docs/reference/capture_seed_oss_chat_template.py from the ORIGINAL "
            "checkpoint chat_template.jinja (jinja2, trim_blocks + lstrip_blocks, "
            "cross-checked == transformers apply_chat_template). "
            "SeedOssChatTemplateParityTests renders `template` through swift-jinja "
            "and asserts equality with each case's `expected`. `cases` are "
            "representative message-set diversity; `boundary_cases` sweep every "
            "thinking-budget reflection-interval tier boundary (±1) plus "
            "0/negative/past-top, proving the checkpoint's integer-keyed dict + "
            "dictsort lookup renders natively for every budget."
        ),

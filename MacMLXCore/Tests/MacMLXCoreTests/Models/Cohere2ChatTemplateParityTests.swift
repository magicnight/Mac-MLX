...
/// Render-parity gate for the built-in Cohere2 / Command R7B chat-template override
/// (`Cohere2ChatTemplate.template`).
///
...
/// swift-jinja 2.3.6 cannot PARSE ("Unexpected token type: closeExpression"), so
/// the whole template fails to compile even though that branch is off the standard
/// conversation path. The override drops that (dead-for-`default`) branch and keeps
/// the standard-conversation `{%- else -%}` branch byte-for-byte. This test proves
/// the drop is semantically identical by rendering the SAME message sets through
/// the OVERRIDE via swift-jinja (the exact engine swift-transformers drives in
/// production, configured identically with `lstripBlocks: true, trimBlocks: true`)
/// and asserting byte-for-byte equality against a fixture captured from the
...
/// of scope for the override and this test.
///
...
/// override; the real-weights `Cohere2SmokeTests` proves the end-to-end path.
///
...
final class Cohere2ChatTemplateParityTests: XCTestCase {
    ...
    /// dropped tool/RAG branch is skipped identically.
    ...
    func testOverrideRendersIdenticallyToOriginal() throws {
        ...
        for testCase in fixture.cases {
            ...
            XCTAssertEqual(
                ...
                "override render diverged from the original `default` template for "
                    + "case '\(testCase.name)'")
        }
    }
}

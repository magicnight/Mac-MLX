...
test("fact, competitor, FAQ, and release registries pass strict validation", async () => {
  ...
  assert.doesNotThrow(() => validateContentHub({ facts, competitors, faqs, releases, pages, macmlxComparisonProfile }, { today: "2026-08-28", maxAgeDays: 45, allowEmptyPages: true }));
  ...
  assert.deepEqual(competitors.map((item) => [item.id, item.verifiedVersion]), [
    ["ollama", "v0.33.1"],
    ["lm-studio", "Bionic 1.1.0"],
    ["omlx", "v0.6.3"],
    ["swama", "v2.3.0"],
    ["swiftlm", "b704"],
  ]);
  assert.equal(faqs.length, 8);
  assert.equal(releases[0].version, "0.9.0");
});
...
test("validation rejects stale dates, mutable release sources, and broken references", async () => {
  ...
  assert.throws(() => validateFacts([stale], { today: "2026-08-28", maxAgeDays: 45 }), /stale fact/);

  const mutable = { ...facts.find((fact) => fact.status === "released"), sourceUrls: ["https://github.com/magicnight/mac-mlx/blob/main/CHANGELOG.md"] };
  assert.throws(() => validateFacts([mutable], { today: "2026-08-28", maxAgeDays: 45 }), /immutable release source/);
...
test("fact support tiers are governed separately from release lifecycle", async () => {
  ...
  assert.throws(
    () => validateFacts([{ ...facts[0], supportTier: "released" }], { today: "2026-08-28", maxAgeDays: 45 }),
...
test("release status lists reject facts from the wrong lifecycle", async () => {
  ...
  for (const [field, wrongFactId, expectedStatus] of [
    ["shippedFactIds", "paged-kv", "released"],
    ["limitationFactIds", "paged-kv", "released"],
    ...
    assert.throws(
      () => validateReleases([mismatch], factIds, { today: "2026-08-28", maxAgeDays: 45, factsById }),
...
test("validation rejects future dates and sources outside official ownership", async () => {
  ...
  assert.throws(() => validateFacts([{ ...facts[0], lastVerified: "2026-08-29" }], { today: "2026-08-28", maxAgeDays: 45 }), /future fact/);
  assert.throws(() => validateFacts([{ ...facts[0], sourceUrls: [...facts[0].sourceUrls, "https://example.com/claim"] }], { today: "2026-08-28", maxAgeDays: 45 }), /unapproved macMLX source/);

  const futureCompetitor = { ...competitors[0], snapshotDate: "2026-08-29" };
  assert.throws(() => validateCompetitors([futureCompetitor], { today: "2026-08-28", maxAgeDays: 45 }), /future competitor snapshot/);
  assert.throws(() => validateCompetitors([{ ...competitors[0], lastVerified: "2026-08-29" }], { today: "2026-08-28", maxAgeDays: 45 }), /future competitor verification/);
  const foreignCompetitor = { ...competitors[0], officialSources: [...competitors[0].officialSources, "https://example.com/ollama"] };
  assert.throws(() => validateCompetitors([foreignCompetitor], { today: "2026-08-28", maxAgeDays: 45 }), /unapproved official source/);

  const futureRelease = { ...releases[0], releaseDate: "2026-08-29" };
  assert.throws(() => validateReleases([futureRelease], new Set(facts.map((fact) => fact.id)), { today: "2026-08-28", maxAgeDays: 45, factsById: new Map(facts.map((fact) => [fact.id, fact])) }), /future release date/);
  assert.throws(() => validateReleases([{ ...releases[0], lastVerified: "2026-08-29" }], new Set(facts.map((fact) => fact.id)), { today: "2026-08-28", maxAgeDays: 45, factsById: new Map(facts.map((fact) => [fact.id, fact])) }), /future release verification/);
});
...
test("page schema rejects free-form source URLs before rendering", async () => {
  ...
  for (const url of ["javascript:alert(1)", "http://example.com", "https://example.com"]) {
    ...
    assert.throws(() => validateContentHub({ facts, competitors, faqs, releases, pages: brokenPages, macmlxComparisonProfile }, { today: "2026-08-28", maxAgeDays: 45 }), /free-form sources are not allowed/);
  }
});

test("released proof requires an immutable tagged source pathname", async () => {
  const { validateFacts } = await import("../lib/content.mjs");
  ...
  assert.throws(() => validateFacts([querySpoof], { today: "2026-08-28", maxAgeDays: 45 }), /immutable release source/);
});
...
test("project and release registries share one current release identity", async () => {
  ...
  const releasePage = pages.find((page) => page.id === "release-v0-9-0");

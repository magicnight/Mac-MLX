...
test("sitemap lists every canonical once with lastmod and reciprocal locale alternates", () => {
  ...
  assert.equal(xml.match(/<url>/g)?.length, 30);
  assert.equal(xml.match(/<loc>/g)?.length, 30);
  assert.equal(xml.match(/hreflang="en"/g)?.length, 30);
  assert.equal(xml.match(/hreflang="zh-Hans"/g)?.length, 30);
  assert.equal(xml.match(/hreflang="x-default"/g)?.length, 30);
  assert.equal(xml.match(/<lastmod>2026-07-19<\/lastmod>/g)?.length, 30);

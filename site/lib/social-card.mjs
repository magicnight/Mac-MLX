...
import { crc32, embedPNGSourceDigest, isPNGSourceDigestChunk, pngSourceDigestPrefix } from "./png-source-digest.mjs";
...
export function embedSocialSourceDigest(buffer, digest) {
  return embedPNGSourceDigest(buffer, digest, "social PNG");
}
...
export function validateSocialPNG(buffer, label = "social PNG", { expectedSourceDigest } = {}) {
  ...
  while (offset < buffer.length) {
    ...
    if (isPNGSourceDigestChunk(type, buffer.subarray(dataStart, crcOffset))) {
      sourceDigests.push(buffer.toString("latin1", dataStart + pngSourceDigestPrefix.length, crcOffset));
    }
    offset = end;
  }

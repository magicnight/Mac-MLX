...
const apiTable = Object.freeze({
  ...
  caption: b("v0.9.0 endpoint families and their explicit boundaries", "v0.9.0 端点家族及明确边界"),
  ...
  rows: Object.freeze({
    en: Object.freeze([
      ...
      Object.freeze(["Rerank", "POST /v1/rerank", "Cross-encoder scoring through a single-logit head; bi-encoder cosine fallback"]),
      Object.freeze(["Audio", "POST /v1/audio/transcriptions, POST /v1/audio/speech", "OpenAI-shaped; an output container macMLX cannot produce is refused, not substituted"]),
    ...
    "zh-Hans": Object.freeze([
      ...
      Object.freeze(["重排", "POST /v1/rerank", "经单 logit 头的交叉编码器打分；双编码器余弦为回退路径"]),
      Object.freeze(["音频", "POST /v1/audio/transcriptions、POST /v1/audio/speech", "OpenAI 形状；macMLX 无法生成的输出容器会被拒绝，而不是替换"]),
...
const apiFactIds = ["openai-compat", "anthropic-messages", "ollama-selected", "mcp-server", "mcp-client-pool", "integrated-tool-routing", "embeddings", "rerank-cross-encoder", "audio-endpoints", "continuous-batching", "structured-output", "api-compat-pack", "kv-cache-quantization"];
const architectureFactIds = ["swift-in-process", "unified-memory", "shared-core", "no-python-default", "tiered-cache", "trie-lcp", "model-pool", "continuous-batching", "fixed-prefill-throttle", "speculative-decoding", "kv-cache-quantization", "silicon-activity-panel", "bottleneck-classifier", "silicon-sampling", "app-controlled-engine", "upstream-correctness-fixes", "paged-kv", "adaptive-memory-guard"];
...
export const pages = Object.freeze([
  page("architecture", "/architecture/", ["How macMLX runs models on Apple Silicon", "macMLX 如何在 Apple 芯片上运行模型"], ["An evidence-backed v0.9.0 guide to the Swift engine, shared core, unified memory, batching, sudoless silicon observability, hardened cache tiers, the controlled MLX build, and roadmap boundaries.", "以 v0.9.0 证据说明 Swift 引擎、共享核心、统一内存、批处理、免 sudo 的硅可观测性、加固的缓存分层、受控 MLX 构建与路线图边界。"], ["The current v0.9.0 engine runs in Swift, while separate processes keep separate in-memory engine instances. Eligibility-gated continuous batching measured 2.5–3.2× throughput with four clients; longest-prefix reuse, speculative decoding, and KV-cache quantization are also released. A sudoless silicon Activity panel attributes the live inference bottleneck, the cold SSD KV tier is byte-bounded, weight-safe, restart-surviving, and non-blocking, and the GUI now links the patched MLX engine. Paged KV, block sharing, and CoW plus an adaptive memory guard remain planned.", "当前 v0.9.0 引擎在 Swift 中运行，不同进程仍各自保有内存中引擎实例。资格门控的连续批处理在四客户端下测得 2.5–3.2× 吞吐量；最长前缀复用、推测解码与 KV 缓存量化也已发布。免 sudo 的硅活动面板归因实时推理瓶颈，冷 SSD KV 层有界、权重安全、可重启保留且非阻塞，GUI 现已链接补丁版 MLX 引擎。分页 KV、块共享、写时复制与自适应内存守卫仍在规划中。"], ["Architecture", "架构"], ["api-compatibility", "models", "releases", "compare-omlx"], [
  ...
  page("api-compatibility", "/api-compatibility/", ["Local API compatibility, endpoint by endpoint", "逐端点说明本地 API 兼容性"], ["A precise v0.9.0 matrix for OpenAI, Anthropic Messages, selected Ollama endpoints, audio, rerank, tool loops, structured output, API controls, and serving boundaries.", "精确列出 v0.9.0 的 OpenAI、Anthropic Messages、选定 Ollama 端点、音频、重排、工具循环、结构化输出、API 控制项与服务边界。"], ["The current v0.9.0 APIs expose released tool loops, structured output, compatibility controls, KV-cache quantization, and OpenAI-shaped audio transcription and speech across scoped local surfaces. Model management under /x/models remains a macMLX extension; Anthropic support is Messages-only; Ollama covers five selected endpoints; rerank is now a joint cross-encoder. Unsupported schemas, parameter combinations, and audio containers return explicit errors.", "当前 v0.9.0 API 在范围明确的本地接口中提供已发布的工具循环、结构化输出、兼容控制项、KV 缓存量化，以及 OpenAI 形状的音频转写与语音合成。/x/models 下的模型管理仍是 macMLX 扩展；Anthropic 仅支持 Messages；Ollama 覆盖五个选定端点；重排现为联合打分的交叉编码器。不支持的 Schema、参数组合与音频容器都会返回明确错误。"], ["API compatibility", "API 兼容性"], ["architecture", "faq", "release-v0-9-0", "compare-ollama"], [
  ...
  page("models", "/models/", ["Choose models by task and memory", "按任务与内存选择模型"], ["A v0.9.0 model hub for checkpoint selection, cache discovery, templates, tested Track G models, OCR recognition, vision, sampling, and support boundaries.", "v0.9.0 模型中心，涵盖检查点选择、缓存发现、模板、实测 Track G 模型、OCR 识别、视觉、采样与支持边界。"], ["The current v0.9.0 model hub includes checkpoint-tested Track G models such as Seed-OSS-36B, with results that are checkpoint-specific, not family-wide performance guarantees. InternLM3-8B remains theoretical only because public checkpoints provide tokenizer.model while the Swift path requires tokenizer.json. macMLX recognizes dedicated OCR checkpoints, so a verified checkpoint like GLM-OCR earns an OCR badge distinct from Vision. Choose by exact architecture, tokenizer, template, memory, and task.", "当前 v0.9.0 模型中心包含经检查点实测的 Track G 模型（如 Seed-OSS-36B），结果仅适用于特定检查点，并非家族级性能保证。InternLM3-8B 仍仅为理论支持，因为公开检查点提供 tokenizer.model，而 Swift 路径需要 tokenizer.json。macMLX 会识别专用 OCR 检查点，因此像 GLM-OCR 这样经过验证的检查点会获得区别于 Vision 的 OCR 徽章。请依据确切架构、分词器、模板、内存与任务进行选择。"], ["Model guides", "模型指南"], ["choosing-a-model", "vision-language-models", "architecture", "faq", "compare-lm-studio"], [
  ...
  page("choosing-a-model", "/models/choosing-a-model/", ["How to choose an MLX model", "如何选择 MLX 模型"], ["A practical v0.9.0 table for architecture, local discovery, parameter count, quantization, context, concurrency, and unified-memory headroom.", "v0.9.0 实用表格，涵盖架构、本地发现、参数量、量化、上下文、并发与统一内存余量。"], ["Start with the task and exact architecture support, then verify tokenizer and checkpoint compatibility, including models discovered in Hugging Face cache roots. Size weights, context cache, runtime overhead, and macOS headroom; use compatible KV-cache quantization only where appropriate. If the estimate approaches physical memory, choose a smaller or more strongly weight-quantized checkpoint.", "先从任务与确切架构支持开始，再核对分词器和检查点兼容性，包括在 Hugging Face 缓存根目录中发现的模型。估算权重、上下文缓存、运行时开销与 macOS 余量；仅在兼容场景使用 KV 缓存量化。若估算接近物理内存，应选择更小或权重量化程度更高的检查点。"], ["Model selection", "模型选择"], ["models", "vision-language-models", "architecture"], [facts("Sizing facts", "规模选择事实", ["unified-memory", "model-pool", "tiered-cache", "kv-cache-quantization", "hf-cache-discovery", "sampling-core", "sampling-expanded"]), choosingTable, sources(["unified-memory", "model-pool", "tiered-cache", "kv-cache-quantization", "hf-cache-discovery", "sampling-core", "sampling-expanded"])]),
  page("vision-language-models", "/models/vision-language-models/", ["Vision-language model support", "视觉语言模型支持"], ["What the 14 detected VLM families mean in v0.9.0, and what to verify for checkpoint, processor, memory, and serving compatibility.", "解释 v0.9.0 中 14 个检测到的 VLM 家族，并说明检查点、处理器、内存与服务兼容性核对项。"], ["The v0.9.0 model library detects exactly 14 VLM model_type families. This is a family-level signal, not universal checkpoint support: image processors, weight variants, quantization, memory use, and serving paths still matter. Dedicated OCR checkpoints such as GLM-OCR load through this same VLM path and earn an OCR badge. Eligibility-gated batching remains dense-text-only, so VLM requests stay serial.", "v0.9.0 模型库检测到恰好 14 个 VLM model_type 家族。这是家族级信号，不代表通用检查点支持；图像处理器、权重变体、量化、内存占用与服务路径仍然重要。像 GLM-OCR 这样的专用 OCR 检查点通过同一 VLM 路径加载，并获得 OCR 徽章。资格门控批处理仅限稠密文本，因此 VLM 请求保持串行。"], ["Vision models", "视觉模型"], ["models", "choosing-a-model", "release-v0-9-0"], [facts("Vision facts", "视觉事实", ["vlm-14-families", "unified-memory", "continuous-batching"]), vlmTable, sources(["vlm-14-families", "unified-memory", "continuous-batching"])]),
  page("faq", "/faq/", ["macMLX questions, answered", "macMLX 常见问题解答"], ["Eight v0.9.0 answers covering platform, installation, Python, models, APIs, privacy, vision, large MoE models, and roadmap status.", "八个 v0.9.0 答案，涵盖平台、安装、Python、模型、API、隐私、视觉、大型 MoE 与路线图状态。"], ["These answers use v0.9.0 as the audited baseline and distinguish released capabilities from planned work. API controls retain provider, schema, and combination boundaries; VLM and large-model answers retain checkpoint-specific caveats. Follow the linked technical pages for details. Every visible claim points to dated official evidence.", "以下答案以 v0.9.0 为审计基线，并区分已发布能力与规划工作。API 控制项保留提供方、Schema 与组合边界；VLM 与大型模型答案保留检查点特定限制。详情请查看相关技术页面。每个可见声明都指向带日期的官方证据。"], ["FAQ", "常见问题"], ["api-compatibility", "models", "releases", "release-v0-5-3"], [Object.freeze({ type: "faq", heading: b("Eight common questions", "八个常见问题"), faqIds: Object.freeze(["platform-installation", "python", "model-selection", "apis", "privacy", "vlm", "large-moe", "roadmap"]) }), sources(["platform-installation", "no-python-default", "unified-memory", "model-pool", "lora", "vlm-14-families", "openai-compat", "anthropic-messages", "ollama-selected", "integrated-tool-routing", "structured-output", "api-compat-pack", "kv-cache-quantization", "audio-endpoints", "rerank-cross-encoder", "swift-in-process", "deepseek-v32", "track-g-tested-models", "continuous-batching", "trie-lcp", "paged-kv", "adaptive-memory-guard", "sampling-expanded", "mtp-drafter-detection"])]),
  page("compare", "/compare/", ["Compare local model tools by factual route", "按事实路线对比本地模型工具"], ["A neutral, dated v0.9.0 overview of macMLX, Ollama, LM Studio, oMLX, Swama, and SwiftLM with official sources.", "以官方来源中立对比 v0.9.0 macMLX、Ollama、LM Studio、oMLX、Swama 与 SwiftLM，并标明日期。"], ["The macMLX v0.9.0 snapshot and the August 2026 competitor snapshots differ in platform reach, runtime, model workflow, interfaces, and focus. The table uses official evidence and declares no score, winner, or universal best choice. Use the dated dimensions to identify the workflow that matches your requirements.", "macMLX v0.9.0 快照与 2026 年 8 月的竞品快照在平台范围、运行时、模型工作流、接口与重点上存在差异。表格采用官方证据，不给出评分、赢家或通用最佳选择。请根据带日期的维度寻找符合自身需求的工作流。"], ["Comparisons", "产品对比"], ["compare-ollama", "compare-lm-studio", "compare-omlx"], [Object.freeze({ type: "comparison", heading: b("Six factual snapshots", "六个事实快照"), competitorIds: Object.freeze(["ollama", "lm-studio", "omlx", "swama", "swiftlm"]) }), sources(comparisonProfileFactIds, ["ollama", "lm-studio", "omlx", "swama", "swiftlm"])]),
  page("compare-ollama", "/compare/ollama/", ["macMLX and Ollama", "macMLX 与 Ollama"], ["A neutral comparison of macMLX v0.9.0 and Ollama v0.33.1 by platform, runtime, models, interfaces, and focus.", "按平台、运行时、模型、接口与重点中立对比 macMLX v0.9.0 与 Ollama v0.33.1。"], ["macMLX v0.9.0 centers on a Swift in-process MLX core for Apple Silicon. Ollama provides a cross-platform service and model workflow, including official MLX support on Apple Silicon. The right fit depends on platform and interface requirements. The comparison below uses August 2026 official snapshots rather than subjective scores.", "macMLX v0.9.0 以 Apple 芯片上的 Swift 进程内 MLX 核心为中心；Ollama 提供跨平台服务与模型工作流，并在 Apple 芯片上官方支持 MLX。选择取决于平台与接口需求。下方对比采用 2026 年 8 月官方快照，而非主观评分。"], ["Comparison", "产品对比"], ["compare", "api-compatibility", "architecture"], [Object.freeze({ type: "comparison", heading: b("Runtime and workflow snapshot", "运行时与工作流快照"), competitorIds: Object.freeze(["ollama"]) }), sources([...comparisonProfileFactIds, "ollama-selected"], ["ollama"])]),
  page("compare-lm-studio", "/compare/lm-studio/", ["macMLX and LM Studio", "macMLX 与 LM Studio"], ["A neutral comparison of macMLX v0.9.0 and LM Studio Bionic 1.1.0 using official sources.", "依据官方来源中立对比 macMLX v0.9.0 与 LM Studio Bionic 1.1.0。"], ["macMLX v0.9.0 focuses on one Swift in-process MLX core for Apple Silicon. LM Studio spans desktop, headless, CLI, SDK, and local-server workflows across platforms and runtimes; its official MLX engine is Python-based and bundles its own Python runtime. The table compares those factual routes without rating either product.", "macMLX v0.9.0 聚焦 Apple 芯片上的一套 Swift 进程内 MLX 核心。LM Studio 跨平台与多运行时覆盖桌面、无头、CLI、SDK 与本地服务工作流；其官方 MLX 引擎基于 Python 并自带 Python 运行时。表格只比较这些事实路线，不对任一产品评分。"], ["Comparison", "产品对比"], ["compare", "models", "architecture"], [Object.freeze({ type: "comparison", heading: b("Runtime and product-surface snapshot", "运行时与产品入口快照"), competitorIds: Object.freeze(["lm-studio"]) }), sources(comparisonProfileFactIds, ["lm-studio"])]),
  page("compare-omlx", "/compare/omlx/", ["macMLX and oMLX", "macMLX 与 oMLX"], ["A neutral comparison of macMLX v0.9.0 and oMLX v0.6.3 by engine, platform requirement, and serving focus.", "按引擎、平台要求与服务重点中立对比 macMLX v0.9.0 与 oMLX v0.6.3。"], ["Both target MLX inference on Apple Silicon and provide native macOS surfaces. macMLX v0.9.0 runs its default engine in-process in Swift; oMLX v0.6.3 uses a Python/FastAPI core with a native SwiftUI menu-bar app and documented continuous batching, paged caching, and multi-token prediction; its v0.6.3 snapshot requires macOS 15.0+ and Python 3.11-3.13.", "两者都面向 Apple 芯片上的 MLX 推理并提供原生 macOS 入口。macMLX v0.9.0 默认引擎在 Swift 进程内运行；oMLX v0.6.3 使用 Python/FastAPI 核心与原生 SwiftUI 菜单栏应用，并记录了连续批处理、分页缓存与多令牌预测；其 v0.6.3 快照要求 macOS 15.0+ 与 Python 3.11-3.13。"], ["Comparison", "产品对比"], ["compare", "architecture", "releases"], [Object.freeze({ type: "comparison", heading: b("Engine and serving snapshot", "引擎与服务快照"), competitorIds: Object.freeze(["omlx"]) }), sources([...comparisonProfileFactIds, "continuous-batching", "paged-kv"], ["omlx"])]),
  page("releases", "/releases/", ["Release status without roadmap blur", "不混淆路线图的版本状态"], ["A current v0.9.0 hub with historical v0.8.0, v0.7.0, v0.6.2, and v0.5.3 records that separates shipped capabilities, limitations, and planned work.", "当前 v0.9.0 版本中心，附 v0.8.0、v0.7.0、v0.6.2 与 v0.5.3 历史记录，区分已交付能力、限制与规划工作。"], ["The current audited release is v0.9.0 from 2026-08-28; v0.8.0, v0.7.0, v0.6.2, and v0.5.3 remain available as historical records. GitHub Releases, immutable tagged changelogs, and the tagged model-support guide are authoritative. This hub renders every record and preserves visible capability labels so later features and future roadmap work are never attributed to a historical release.", "当前审计版本是 2026-08-28 发布的 v0.9.0；v0.8.0、v0.7.0、v0.6.2 与 v0.5.3 保留为历史记录。GitHub Releases、不可变标签更新日志与标签版模型支持指南是权威来源。本中心呈现每条记录并保留可见能力标签，避免把后续功能或未来路线图归入某个历史版本。"], ["Releases", "版本"], ["release-v0-9-0", "release-v0-8-0", "release-v0-7-0", "release-v0-6-2", "release-v0-5-3", "architecture", "api-compatibility"], [Object.freeze({ type: "release", heading: b("Current and historical releases", "当前与历史版本"), releaseIds: Object.freeze(["v0-9-0", "v0-8-0", "v0-7-0", "v0-6-2", "v0-5-3"]) }), sources(releaseSourceFactIds, [], ["v0-9-0", "v0-8-0", "v0-7-0", "v0-6-2", "v0-5-3"])]),
  page("release-v0-9-0", "/releases/v0-9-0/", ["macMLX v0.9.0", "macMLX v0.9.0"], ["The current release: in-process audio, a real cross-encoder reranker, and the first GUI that links macMLX's patched MLX engine.", "当前版本：进程内音频、真正的交叉编码器重排器，以及首个链接 macMLX 补丁版 MLX 引擎的 GUI。"], ["v0.9.0 is the current audited baseline. It adds in-process speech to text and text to speech on MLX, replaces the bi-encoder rerank MVP with a joint cross-encoder scored through a single-logit head, and is the first build whose GUI actually links macMLX's patched MLX engine. The audio and reranker paths are code-complete and unit-tested, but unvalidated against real checkpoints.", "v0.9.0 是当前审计基线。它新增基于 MLX 的进程内语音转文字与文字转语音，将双编码器重排 MVP 换成经单 logit 头联合打分的交叉编码器，并且是首个 GUI 真正链接 macMLX 补丁版 MLX 引擎的构建。音频与重排路径已完成代码并有单元测试，但尚未在真实检查点上验证。"], ["Release", "版本"], ["releases", "api-compatibility", "release-v0-8-0"], [Object.freeze({ type: "release", heading: b("Current v0.9.0 release", "当前 v0.9.0 版本"), releaseIds: Object.freeze(["v0-9-0"]) }), sources(releaseFactIds("v0-9-0"), [], ["v0-9-0"])]),
  page("release-v0-8-0", "/releases/v0-8-0/", ["macMLX v0.8.0 (historical)", "macMLX v0.8.0（历史版本）"], ["A historical record of what shipped on 2026-07-19: a bounded, weight-safe, restart-surviving, and non-blocking cold SSD KV cache.", "2026-07-19 已交付内容的历史记录：有界、权重安全、可重启保留且非阻塞的冷 SSD KV 缓存。"], ["v0.8.0 is an explicitly historical audited record. It hardened the tiered SSD KV cache so its cold tier became byte-bounded and pruned oldest-first, weight-identity-guarded so a swap could not serve stale KV, backed by a persisted cross-session index, and written off the cache actor. Its tagged release and changelog remain authoritative; the later v0.9.0 audio and reranker work is not part of it.", "v0.8.0 是明确标注的历史审计记录。它对分层 SSD KV 缓存进行加固：冷层按字节约束并最旧优先清理、以权重身份守护使替换后无法提供陈旧 KV、由持久跨会话索引支撑，并将写入迁出缓存 actor。其标签版发布记录与更新日志仍是权威依据；后续 v0.9.0 的音频与重排工作不属于此记录。"], ["Historical release", "历史版本"], ["releases", "models", "release-v0-7-0"], [Object.freeze({ type: "release", heading: b("Historical v0.8.0 scope", "历史 v0.8.0 范围"), releaseIds: Object.freeze(["v0-8-0"]) }), sources([], [], ["v0-8-0"])]),
  page("release-v0-7-0", "/releases/v0-7-0/", ["macMLX v0.7.0 (historical)", "macMLX v0.7.0（历史版本）"], ["A historical record of what shipped on 2026-07-18: a sudoless silicon Activity panel, live inference-bottleneck attribution, benchmark bottleneck attribution, and OCR-model recognition.", "2026-07-18 已交付内容的历史记录：免 sudo 的硅活动面板、实时推理瓶颈归因、基准瓶颈归因与 OCR 模型识别。"], ["v0.7.0 is an explicitly historical audited record. It covers the sudoless silicon-metrics Activity panel, the in-process inference-bottleneck classifier, benchmark bottleneck attribution, and OCR-model recognition, without importing the later v0.8.0 SSD KV-cache hardening. Its tagged GitHub release, changelog, and model-support guide remain authoritative; this page intentionally keeps only the v0.7.0 release block, its limitations, and the planned work recorded at that tag.", "v0.7.0 是明确标注的历史审计记录。它涵盖免 sudo 的硅指标活动面板、进程内推理瓶颈分类器、基准瓶颈归因与 OCR 模型识别，不引入后续 v0.8.0 的 SSD KV 缓存加固。其 GitHub 标签发布、更新日志与模型支持指南仍是权威依据；本页有意只保留 v0.7.0 版本块、其限制与该标签时记录的规划工作。"], ["Historical release", "历史版本"], ["releases", "models", "release-v0-6-2"], [Object.freeze({ type: "release", heading: b("Historical v0.7.0 scope", "历史 v0.7.0 范围"), releaseIds: Object.freeze(["v0-7-0"]) }), sources([], [], ["v0-7-0"])]),
